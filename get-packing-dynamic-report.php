...
if(isset($_SESSION['mis_key']) && $_SESSION['mis_key']==1)
{
	if(isset($_POST['key']) && $_POST['key']=='147')
	{
		...
		<table class="table" id="packing-table">
          ...
          $sql="SELECT packing.pump_sno AS pump_sno,packing.date AS date,packing.time AS time,packing.shift AS shift,master_scheduling.fg AS fg_no,master_scheduling.seq_no AS sequence_no,master_scheduling.fg_part_no AS production_order_no,master_scheduling.customer AS customer,master_scheduling.type AS pump_type,master_scheduling.tpi AS tpi,packing.operator_name AS operator_name from packing INNER JOIN master_scheduling ON master_scheduling.serial_no=packing.pump_sno WHERE packing.date BETWEEN '$startdate' AND '$enddate' ORDER BY packing.id ASC";
